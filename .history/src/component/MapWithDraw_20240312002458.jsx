...
export default function MapWithDraw() {
  ...
  useEffect(() => {
    // Initialize visibility state for each stored layer
  ...
  const saveLayer = () => {
    ...
    if (drawnPolygons.length > 0) {
      ...
      setVisibleLayers((prevState) => ({
        ...prevState,
        [storedLayers.length]: true, // Set visibility for new layer
      }));
  ...
  const toggleStoredLayer = (index) => {
    ...
    if (!visibleLayers[index]) {
      draw.current.add(storedLayers[index]); // Add layer if not visible
    } else {
      draw.current.delete(storedLayers[index].id); // Remove layer if visible
    }
  };
