...
export default function MapWithDraw() {
  ...
  const [storedLayer, setStoredLayer] = useState(null); // State to store the current layer
  ...
  const saveLayer = () => {
    setStoredLayer(polygons);
  };

  // Function to toggle the stored layer on and off
  const toggleStoredLayer = () => {
    if (storedLayer) {
      draw.current.add(storedLayer);
      setStoredLayer(null);
    } else {
  ...
  return (
    <div>
      ...
      <button onClick={toggleStoredLayer}>
        {storedLayer ? "Hide Saved Layer" : "Show Saved Layer"}
      </button>
      <div>
