...
export default function MapWithDraw() {
  ...
  // Function to toggle a specific layer for saving
  const toggleLayer = (layerId) => {
    setLayersEnabled((prevLayers) => ({
      ...prevLayers,
      [layerId]: !prevLayers[layerId], // Toggle the value for the given layer ID
    }));
  };
  ...
  return (
    <div>
      ...
      <div className="sidebar">
        <h3>Toggle Layers:</h3>
        {Object.keys(draw.current?.getAll() || {}).map((layerId) => (
          <div key={layerId}>
            <input
              type="checkbox"
              checked={layersEnabled[layerId]}
